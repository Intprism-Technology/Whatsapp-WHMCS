...
access.execute("SELECT * FROM tblinvoices WHERE status = 'Unpaid' AND date = CURDATE()")
...
for x in resultInvoice:
    ...
    for user in resultUser:
        ...
        if invoiceNumber == "":
            invoiceNumber = x[0]
        messageToSend = template_message.invoice_unpaid.format(firstName = firstName,lastName = lastName,phone = phone, invoiceNumber = invoiceNumber, duedate = str(x[4]), duetotal= x[13])
